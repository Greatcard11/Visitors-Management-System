...
def page_register():
    ...
    with st.form("visitor_registration_form", clear_on_submit=False):
        ...
        with c1:
            ...
            host_options = [h["name"] for h in db.get_hosts(dept)] if dept else hosts
            person_to_visit  = st.selectbox("Person to Visit *", [""] + host_options)
                ...
                with st.container():
                    ...
                    <div class="visitor-card {card_cls}">
                        ...
                        <div style="margin-top:8px;font-size:.85rem;color:#475569">
                            ...
                            📋 {v['purpose']} &bull; ⏱️ {v['expected_duration'] or 'N/A'} &bull;
        ...
        if checkout_btn and manual_input.strip():
            ...
            if not visitor:
                ...
                <div class="form-panel" style="border-left:4px solid #3b82f6">
                    <h3 style="color:#1a2744;margin:0 0 8px">🚪 {visitor['full_name']}</h3>
                    <p style="color:#64748b;margin:0">
                        Visitor No: <strong>{visitor['visitor_number']}</strong><br>
                        Department: {visitor['department']} &rarr; {visitor['person_to_visit']}<br>
                        Entry: {visitor['entry_date']} {visitor['entry_time']}<br>
                        Exit: {now.strftime('%Y-%m-%d')} {now.strftime('%H:%M:%S')}<br>
                        <strong>Total Duration: {duration}</strong>
                    </p>
                </div>
                """, unsafe_allow_html=True)
        elif checkout_btn:
